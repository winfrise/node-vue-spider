  ...
  let urls = await(async function (page) {
    ...
    for(let i = startNum; i <= endNum; i++) {
      ...
      for (let j = 1; j <= totalPage; j++) {
        ...
        let links = await methods.getDoms(page, 'a.photo-thumb', (nodes) => nodes.map(n => n.href))
        console.log('links', links)
        ;(async function ()  {
          for (let i = 0; i < links.length; i ++) {
            console.log(links[i])
            await methods.gotoPage(page, {url: links[i]}, 'networkidle0')
            let imgSrc = await methods.getDom(page, '.fotorama__active .fotorama__img', node => node.src)
            console.log(imgSrc)
